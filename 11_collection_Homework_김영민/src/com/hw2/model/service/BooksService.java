...
public class BooksService {
		...
		do {
			...
			try {
				...
				case 2:
					...
					break;
				case 3:
					System.out.println(fixBook());
					break;
				case 4:
					System.out.println(dellBook());
					break;
				case 5:
					System.out.println(starkBookAdd());
					break;
				case 6:
					System.out.println(starkBookDell());
					break;
				case 7:
					starBookAll();
					break;
				...
				default:
					System.out.println("알맞은 번호를 넣어주세요");
	...
	/**
	 * 전도서 출력 기능! 도서 조회 모든 도서를 조회 시킨다는 느낌 ?
	 * 
	 * 
	 */
	public void allBook() {
	 ...
	 * 도서 수정 하는 기능!
	 * 
	 * 수정할 도서 번호를 입력하세요 : 도서번호를 입력받으면 해당객체의 정보를 출력
	 * 
	...
	public String fixBook() {
		...
		/*
		 * 도서정보가 bookList 에 하나라도 있는지 검사, 없다면 "입력된 도서정보가 없습니다" 문자열 반환
		 * 
		 * - 인덱스 번호로 도서를 수정하려고 하는데 , 입력된 숫작 0보다 작은지 검사, 작다면 "음수는 입력할수 없습니다" 반환
		 * 
		 * - booktList 범위 내 인덱스 번호인지 검사 , 초과했다면 "범위를 넘어선 값을 입력할 수 없습니다" 반환
		 * 
		 * -수정할 때 1. index 번째에 저장된 도서정보 수정 전 정보를 출력
		 * 
		...
		for (Books b : bookList) {
			...
			case 3:
				System.out.print("수정할 가격명을 입력하세요");
	...
	/**
	 * 있는 도서 삭제시키는 기능 도서 정보가 Booklist 에 하나라도 있는지 검사, 없다면 "입력된 도서정보가 없습니다" 문자열 반환
	 * 
	 * - 인덱스 번호로 학생을 수정하려고 하는데 , 입력된 숫작 0보다 작은지 검사, 작다면 "음수는 입력할수 없습니다" 반환
	 * 
	 * - BookList 범위 내 인덱스 번호인지 검사 , 초과했다면 "범위를 넘어선 값을 입력할 수 없습니다" 반환
	 *
	 * 수정과 같은 검사후 삭제시 "정말 삭제하시겠습니까? (Y/N) Y를 입력했따면 해당 인덱스에 있는 객체 삭제후 " 000님의 정보가
	 * 제거되었습니다" 리턴 N을 입력했다면 "취소" 리턴
	 * 
	 */
	...
	/**
	 * 즐겨찾기 추가기능 그냥 starList를 하나만들었는데 여기다가 추가시키면 될듯 ??
	 * 
	 * 
	 */
