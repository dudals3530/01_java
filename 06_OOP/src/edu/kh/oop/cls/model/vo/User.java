...
public class User {
	...
	// 기본 생성자 자동완선 단축키: ctrl + space + enter
	// 기본 생성자 (-> 매개변수 없는 생성자)
	public User() {
		// 기능
	...
	// 매개변수 생성자
	// 변수 : 매개변수, Overloading(오버로딩), this->참조변수

	// 매개변수 생성자 자동와넝
	// alt + shift + s - > Generate Consturctor using Fields.. - > generate

	// ** 매개변수 : 생성자나 메서드 호출 시 () 안에 작성되어
	// 전달되어지는 값을 저장하고있는 변수
	// -> 전달받은 값을 저장하고있는 매개체 역할의 변수
	public User(String userId, String userPw) {
		System.out.println("매개변수이용한 user 객체 생성");
	...
	public User(String userId, String userPw, String userName, int userAge, char userGender) {
		// System.out.println("매개변수5개짜리생성자");

		// this() 생성자
		// 같은 클래스의 다른 생성자를 호출할때 사용
		// 생성자 내에서 반드시 첫번쨰 줄에 작성해야한다!
		// 왜 사용하는가?
	...
	// [오버로딩 조건]
	// 1) 메서드의 이름이 동일
	// 2) 매개변수의 개수, 타입(자료형), 순서중 1개라도 달라야함.

	// public User() {} // 기본생성자가 이미 작성되어있음

	public User(String userId) {
	} // 매개변수의 개수가 같은 생성자가 없음
	// - > 오버로딩 성립

	public User(int UserAge) { // 매개변수의 개수는 같지만 타입이다름.
		// - > 오버로딩 성립

	}

	public User(String userId, int userAge) {
		// 매개변수의 개수와 동일한 것이있으나, 하나의 타입이 다름
		// - > 오버로딩 성립
	}

	public User(int userAge, String userId) {
		// 매개변수의 갯수, 타입은 같지만 순서가 다름
		// - > 오버로딩 성립
	}
	...
	// 1. 만약에 오타가 있거나, 부모 클래스에 없는 메서드를 잘못 작성했을 경우컴파일 오류발생
	// 2. 이 메서드가 부모 클래스를 재정의 했다는 것을 하눈에 알수 있음
	@Override
	...
	// - > 보통 객체의 필드값을 출력하는 용도로 오버라이딩 해서 사용함.
	// @ - > (어노테이션) : 컴파일러 인식용 주석
	 ...
	 * 
	 * - 생성자 작성 규칙 1) 생성자의 이름은 반드시 클래스명과같아야 한다. 2) 반환형이 존재 하지 않다.
	 * 
	 * - 생성자 종류 1) 기본 생성자 2) 매개변수 생성자
	 *
	 * 
	 * 
	 * 
